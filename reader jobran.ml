...
and nt_line_comment str =  
    let nt1 = nt_end_of_line_or_file in
    let nt2 = star nt_any in
    let nt3 = word ";" in
    let nt1 = unitify ( caten nt3 (caten nt2 nt1)) in 
    nt1 str
...
and nt_symbol_char str =
    ...
    let nt_final = disj_list [nt1;nt2;nt3;nt4;nt5;nt6;nt7;nt8] in
    nt_final str 
...
and nt_improper_list str = 
    ...
    let nt1 = caten nt1  (char ')') in
    let nt1 = pack nt1 (fun ((((left, sexpr1),p),sexpr2),right)-> 
                  put_pair sexpr1) in nt1 str
 ...
 and nt_quoted_forms str = 
    let nt1 = pack (disj_list [nt_quoted;nt_quasi_quoted
                               ;nt_unquoted;nt_unquote_and_spliced]) (fun(quo, sexp) -> match quo with
                                                                                     |['\''] -> ScmPair (ScmSymbol "Quoted" ,ScmPair (sexp, ScmNil))
                                                                                     |['`'] -> ScmPair (ScmSymbol "QuasiQuoted" , ScmPair (sexp, ScmNil))
                                                                                     |[','] -> ScmPair (ScmSymbol "Unquoted" , ScmPair (sexp, ScmNil))
                                                                                     |[',';'@'] -> ScmPair (ScmSymbol "UnquoteAndSpliced" , ScmPair (sexp, ScmNil))
                                                                                     | _ -> ScmNil) in
    nt1 str
